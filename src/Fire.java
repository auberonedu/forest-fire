...
public class Fire {
    ...
    public static List<int[]> timeToBurn(char[][] forest, int r, int c, int time) {
        ...
        for (int[] direction : directions) {
            ...
            if (newR >= 0 && newC >= 0 && newR < forest.length && newC < forest[0].length
                    && forest[newR][newC] == 't') {
                // setting it to a non tree so we can skip over and not do bfs on it
                forest[newR][newC] = '.';

                // add this tree to our newTrees cause its on fire now,
                // the plus 1 is adding another seccond from the current time. Essentially each
                // new tree adds 1 second to it.
